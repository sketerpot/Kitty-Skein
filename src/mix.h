...
/* Take A and rotate it SHIFT bits to the left or right. Both A and
   SHIFT should be simple variables or constants, for safety. */
#define ROTATE_LEFT(A, SHIFT) (A << SHIFT | A >> (sizeof(A) * 8 - SHIFT))
#define ROTATE_RIGHT(A, SHIFT) (A >> SHIFT | A << (sizeof(A) * 8 - SHIFT))
...
#define MIXINV(ROTCONST, X0, X1, Y0, Y1) do { \
  X1 = ROTATE_RIGHT((Y0 ^ Y1), ROTCONST); \
  X0 = Y0 - X1; \
  } while (0);
